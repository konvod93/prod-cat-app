...
import { ShoppingCartIcon, CheckIcon, ArrowLeftIcon } from '@heroicons/react/24/outline';
...
const renderStars = (rating) => {
  ...
  for (let i = 0; i < 5 - Math.ceil(rating); i++) stars.push(<span key={`e${i}`} className="text-gray-300">★</span>);
  return stars;
};
...
export default function ProductPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      {(product.detailedDescription || (product.specifications && Object.keys(product.specifications).length > 0)) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">

          {/* Подробное описание */}
          {product.detailedDescription && (
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-6">
              <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">Описание</h2>
          ...
          {product.specifications && Object.keys(product.specifications).length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-6">
              <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">Характеристики</h2>
